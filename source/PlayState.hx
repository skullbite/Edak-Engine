...
class PlayState extends MusicBeatState
{
    ...
    function set_camOn(val:OneOfTwo<Array<Float>, String>) {
		...
		var camCall = HFunk.doDaCallback(CAMERA, [val]);
		if (camCall.contains(HVars.STOP)) return camOn = val;
	...
	override function sectionHit() {
        ...
        if (songStarted && moveCamera) camOn = SONG.notes[curSection].mustHitSection ? "bf" : "dad";
    }
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			if (tmr.elapsedLoops % dad?.bopSpeed == 0) dad?.dance();
			if (tmr.elapsedLoops % gf?.bopSpeed == 0) gf?.dance();
			if (tmr.elapsedLoops % bf?.bopSpeed == 0) bf?.dance();
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.EIGHT) {
			...
			if (FlxG.keys.pressed.CONTROL) {
				isPlayer = true;
				char = bf != null ? bf.curCharacter : null;
			}
		...
		if (generatedMusic)
			{
				...
				*/
				...
				{	
					...
					if (![0, 1, 2, 3].contains(daNote.noteData)) {
						notes.remove(daNote, true);
						daNote.destroy();
						break;
					}
					...
					var strumLineMid = playerStrums.members[daNote.noteData].height;
					...
					if (FlxG.save.data.downscroll)
						{
							if (daNote.mustPress)
								...
								daNote.y = (cpuStrums.members[Math.floor(Math.abs(daNote.noteData))].y + 0.45 * (Conductor.songPosition - daNote.strumTime) * FlxMath.roundDecimal(FlxG.save.data.scrollSpeed == 1 ? SONG.speed : FlxG.save.data.scrollSpeed, 2));
							if(daNote.isSustainNote)
							{
								...
								{
									if((!daNote.mustPress || daNote.wasGoodHit || !daNote.isSustainEnd ||  daNote.prevNote.wasGoodHit && !daNote.canBeHit) && daNote.y - daNote.offset.y * daNote.scale.y + daNote.height >= (strumLineMid + Note.swagWidth / 2))
									{
										// Clip to strumline
						...
						}else
						{
							...
							if(daNote.isSustainNote && !daNote.noHit)
							{
								...
								if(!FlxG.save.data.botplay)
								{
									if((!daNote.mustPress || daNote.wasGoodHit || !daNote.isSustainEnd || daNote.prevNote.wasGoodHit && !daNote.canBeHit) && daNote.y + daNote.offset.y * daNote.scale.y <= (strumLineMid + Note.swagWidth / 2))
									{
										// Clip to strumline
		...
		private function keyShit():Void // I've invested in emma stocks
				{
					...
					notes.forEachAlive(function(daNote:Note)
					{
						if(FlxG.save.data.downscroll && daNote.strumTime > Conductor.songPosition - Conductor.safeZoneOffset ||
	...
	*/
	...
	override function stepHit()
	{
		...
		if (FlxG.sound.music.time >= Conductor.songPosition + 30 || FlxG.sound.music.time <= Conductor.songPosition - 30)
		{
			resyncVocals();
		}
