...
class FreeplayState extends MusicBeatState
{
	...
	var jukeboxSongStatus(null, set):String = "waiting";
	var jukeboxControl:String = "Play Song";
	var jukeboxStatus:String = "";
	function set_jukeboxControl() updateInfoTxt();
	function set_jukeboxSongStatus(val:String) {
		switch (val) {
			case "waiting": jukeboxControl = "Play Song";
			case "loading": jukeboxControl = "Fetching Song...";
			case "error": jukeboxStatus = "Error! Couldn't fetch " + songs[curSelected].songName;
			case "playing": 
				jukeboxControl = curSelected == currentlyPlaying ? "Pause" : "Play Song";
				jukeboxStatus = "";
		}
		return jukeboxSongStatus = val;
	}*/
	...
	override function create()
	{
		...
		bg = new FlxSprite().loadGraphic(Paths.image('menuBGs/${Settings.get("darkBg") ? 'menuBlack' : 'menuDesat'}'));
		add(bg);
		...
		infoTxt = new FlxText(2, coolBotomInfo.y + 2, 0, '[SPACE] $jukeboxControl');
	...
	override function update(elapsed:Float)
	{
		...
		/*if (spaced) {
			if (currentlyPlaying == curSelected && jukeboxInst.playing) {
				jukeboxInst.destroy();
				jukeboxVocals.destroy();
				jukeboxControl = 'Play';
			}
			currentlyPlaying = curSelected;
			FlxG.sound.music.pause();

			var songSuffix = difficultyData.get(
				curDifficultyArray[curDifficulty].toLowerCase()
				)
				.loadsDifferentSong 
			? 
			curDifficultyArray[curDifficulty].toLowerCase() 
			...
			jukeboxSongStatus = "loading";
			// asynchronus loading my beloved
			Sound.loadFromFile(Paths.inst(songs[curSelected].songName, songSuffix, true))
			.onError(s -> jukeboxSongStatus = "error")
			.onComplete(s -> {
				jukeboxInst.loadEmbedded(s);
				if (FileSystem.exists(Paths.voices(songs[curSelected].songName, songSuffix, true))) Sound.loadFromFile(Paths.voices(songs[curSelected].songName, songSuffix, true)).onComplete(v -> {
					jukeboxVocals.loadEmbedded(v);
					jukeboxInst.play(true);
					jukeboxVocals.play(true);
				});
				else jukeboxInst.play(true);
				jukeboxInst.onComplete = () -> {
					jukeboxInst.stop();
					jukeboxVocals.stop();
				}
				jukeboxSongStatus = "playing";
				jukeboxStatus = '| Now Playing: ${songs[curSelected].songName}';
				updateInfoTxt();
			});
			#if MODS
			Paths.curModDir = null;
	...
	// function updateInfoTxt() infoTxt.text = '[SPACE] $jukeboxControl $jukeboxStatus';
