...
class SoundFrontEnd
{
	...
	public function playMusic(embeddedMusic:FlxSoundAsset, volume = 1.0, looped = true, ?group:FlxSoundGroup):Void
	{
        ...
        if (embeddedMusic is String && !Assets.exists(embeddedMusic) && FileSystem.exists(embeddedMusic)) embeddedMusic = Sound.fromAudioBuffer(AudioBuffer.fromFile(embeddedMusic));
