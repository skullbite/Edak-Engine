...
function onCutscene() {
    var lunchboxScary = FlxG.sound.play(Paths.music('ingame/weeb/LunchboxScary'), 0, true);
    ...
    new FlxTimer().start(0.3, t -> {
		...
		FlxG.sound.play(Paths.sound('weeb/Senpai_Dies'), 1, false, null, true, () -> {
            FlxG.camera.stopFX();
            game.remove(blue);
            game.remove(senpaiEvil);
            game.openDialogueBox("thorns", CoolUtil.coolTextFile(Paths.txt('dialogue/thornsDialogue')), () -> {
